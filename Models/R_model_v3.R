  ...
  for(g in 1:generation){
    ...
    if(length(mig) != 0){
      for(i in 1:length(mig)){
        if(subpop == 0){
          c <- mig[i] - i + 1
          sub_sex <- c(sub_sex, main_sex[c])
          sub_locus1 <- cbind(sub_locus1, main_locus1[,c])
          sub_locus2 <- cbind(sub_locus2, main_locus2[,c])
          sub_locus3 <- cbind(sub_locus3, main_locus3[,c])
          main_sex <- main_sex[-c]
          main_locus1 <- main_locus1[,-c]
          main_locus2 <- main_locus2[,-c]
          main_locus3 <- main_locus3[,-c]
        }else{
          c <- mig[i]
          a <- sample(1:length(sub_sex), 1)
          b <- main_sex[c]
          main_sex[c] <- sub_sex[a]
          sub_sex[a] <- b
          b <- main_locus1[,c]
          main_locus1[,c] <- sub_locus1[,a]
          sub_locus1[,a] <- b
          b <- main_locus2[,c]
          main_locus2[,c] <- sub_locus2[,a]
          sub_locus2[,a] <- b
          b <- main_locus3[,c]
          main_locus3[,c] <- sub_locus3[,a]
          sub_locus3[,a] <- b
        }
      }
    }
