...
extinction <- vector("numeric", run)
for(i in 1:nrow(all_popsize_sub)){
  build <- 0
  extinct <- 0
  for(j in 1:ncol(all_popsize_sub)){
    if(all_popsize_sub[i,j] == maxsubpop){
      build <- 1
      next
    }
    if(all_popsize_sub[i,j] == 0 && build == 1){
      extinct <- extinct + 1
      build <- 0
    }
  }
  extinction[i] <- extinct
}


max_len <- max(length(parthe_main), length(parthe_sub), length(neutral_main), length(neutral_sub), length(nosub), length(interact), length(extinction))
...
extinction_pad <- pad_vector(extinction, max_len)
