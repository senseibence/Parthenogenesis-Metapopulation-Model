# Parthenogenesis Metapopulation Model v2
# added custom random number generator

class LCG:
    def __init__(self, seed, a=109, c=853, m=2**20):
        self.state = seed
        self.a = a
        self.c = c
        self.m = m
...
for run in range(total_runs):
    ...
    for i in range(1, numgens+1):
        ...
        for j in range(1, popsize+1):
            ...
            # locus 2 allele 1
            z = round(lcg.random(),6)
            if (z < mutation): loc2allele1[j] = (round(lcg.random(),6))/2
            if (loc2allele1[j] > maxparth): maxparth = loc2allele1[j]

            # locus 2 allele 2
            z = round(lcg.random(),6)
            if (z < mutation): loc2allele2[j] = (round(lcg.random(),6))/2
            if (loc2allele2[j] > maxparth): maxparth = loc2allele2[j]

            # locus 3 allele 1
            z = round(lcg.random(),6)
            if (z < mutation): loc3allele1[j] = (round(lcg.random(),6))/2

            # locus 3 allele 2
            if (z < mutation): loc3allele2[j] = (round(lcg.random(),6))/2
        ...
        for a in range(1, popsize+1):
            ...
            for b in range(1, x+1):
                while (True):
                    zz = int((round(lcg.random(),6))*(popsize+1))
        ...
        for a in range(1, sexualmatingscount):
            ...
            for b in range(1, max+1):
                ...
                x = int((round(lcg.random(),6))*2)
                y = round(lcg.random(),6)
                z = round(lcg.random(),6)
