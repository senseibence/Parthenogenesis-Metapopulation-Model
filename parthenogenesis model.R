...
repro_method <- function(sex, locus2, mate){
  ...
  for(i in 1:length(sex)){
    if(sex[i] == 0 && mate[i] != 0){
      if(sample(1:100, 1)/100 > sum(locus2[,i])){
        sexual <- c(sexual, i)
      }else{
        par_sex <- c(par_sex, i)
      }
    }
    if((sex[i] == 0) && (mate[i] == 0) && (sample(1:100, 1)/100 <= sum(locus2[,i]))){
      par <- c(par, i)
    }
  }
...
for(n in 1:run){
  ...
  f_locus1 <- vector("numeric", 0)
  f_locus2_main <- vector("numeric", 0)
  f_locus3_main <- vector("numeric", 0)
  f_locus2_sub <- vector("numeric", 0)
  f_locus3_sub <- vector("numeric", 0)
  par_num_main <- vector("numeric", 0)
  par_num_sub <- vector("numeric", 0)
  popsize_sub <- vector("numeric", 0)
  sexratio_main <- vector("numeric", 0)
  sexratio_sub <- vector("numeric", 0)
  
  for(g in 1:generation){
    par_sub <- 0
    main_locus1 <- mutation_bi(main_locus1, Rmut)
    main_locus2 <- mutation_pro(main_locus2, Rmut)
    main_locus3 <- mutation_pro(main_locus3, Rmut)
    ...
    if(subpop != 0){
      ...
      sub_locus2 <- mutation_pro(sub_locus2, Rmut)
      sub_locus3 <- mutation_pro(sub_locus3, Rmut)
    ...
    print(sum(main_locus2))
    print(sum(sub_locus2))
    f_locus1 <- c(f_locus1, (sum(main_locus1) + sum(sub_locus1))/(2*ncol(main_locus1) + 2*ncol(sub_locus1)))
    f_locus2_main <- c(f_locus2_main, sum(main_locus2))
    f_locus3_main <- c(f_locus3_main, sum(main_locus3))
    par_num_main <- c(par_num_main, main_offspring[[5]])
    popsize_sub <- c(popsize_sub, subpop)
    sexratio_main <- c(sexratio_main, 1 - (sum(main_sex)/length(main_sex)))
    if(subpop != 0){
      f_locus2_sub <- c(f_locus2_sub, sum(sub_locus2))
      f_locus3_sub <- c(f_locus3_sub, sum(sub_locus3))
      par_num_sub <- c(par_num_sub, par_sub)
      sexratio_sub <- c(sexratio_sub, 1 - (sum(sub_sex)/length(sub_sex)))
    }else{
      f_locus2_sub <- c(f_locus2_sub, NA)
      f_locus3_sub <- c(f_locus3_sub, NA)
      par_num_sub <- c(par_num_sub, NA)
      sexratio_sub <- c(sexratio_sub, NA)
    }
  }
...
par(mfrow = c(1, 2))
plot(x = c(0,generation), y = c(0,100), type = "n", xlab = "generation", ylab = "frequency", main = "mainpopulation parthenogenesis locus")
for(i in 1:run){
  lines(all_locus2_main[i,], col = i)
}

plot(x = c(0,generation), y = c(0,100), type = "n", xlab = "generation", ylab = "frequency", main = "subpopulation parthenogenesis locus")
for(i in 1:run){
