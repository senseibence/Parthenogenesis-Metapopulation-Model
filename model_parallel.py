...
def run_simulation(run):
    ...
    for i in range(1, numgens+1):
        ...
        for j in range(1, popsize+1):
            ...
            z = random.random()
            if (z < mutation):
                loc2allele1[j] = 0.5  # Simulate mutation
            if (loc2allele1[j] > maxparth): maxparth = loc2allele1[j]

            # Locus 2 allele 2
            z = random.random()
            if (z < mutation):
                loc2allele2[j] = 0.5  # Simulate mutation
            if (loc2allele2[j] > maxparth): maxparth = loc2allele2[j]

            # Locus 3 allele 1
            z = random.random()
            if (z < mutation): loc3allele1[j] = 0.5  # Simulate mutation

            # Locus 3 allele 2
            z = random.random()
            if (z < mutation): loc3allele2[j] = 0.5  # Simulate mutation
